...
|---|---|---|
| `_KB_INDEX.md` | ASA classic full reference | Points ASA NAT, VPN, ACL, and packet-flow topics to `All_combined_part3.md` |
```
```md
...
|---:|---|---|---|---|
| 1 | Confirm ASA routed baseline is working | ASA | `show interface ip brief` | Inside and outside interfaces are up/up |
```
```
# Source_Basis
# <Topic>_Mental_Model
# <Topic>_Configuration_Checklist
# <Topic>_Skeleton
# <Topic>_Verification_Commands
# <Topic>_Rollback
# <Topic>_Failure_Checks
# index of each title throughout note, not in table format
```

##### request tables using the following scalpel 
Okay can you curate a Copy/paste-ready operational checklist table of bgp configuration steps


##### curate note list per virl list
```
can you curate the proper note order for the following and ensure the correct mental models are being addressed. Make mechanism notes, then attach the related lab names under each note.:
```


##### money new convo pickup

$$
Okay can you curate a Copy/paste-ready operational checklist table of ASA_Hairpin_Internal_Server_Public_IP configuration steps

  

Look back at previous convos in this project, specifically the last convo at how I want structure and formats.

  

Here is how the last out put looks, want the tables in Md code blocks:

  

Use this as the full ASA_NAT_Exemption_Identity_NAT.md note.
...
|---|---|---|  
| `_KB_INDEX.md` | ASA classic full reference | Points ASA NAT, VPN, ACL, and packet-flow topics to `All_combined_part3.md` |  
| `All_combined_part3.md` lines 210182-210184 | Identity NAT definition | Identity NAT bypasses address translation so ASA does not change source or destination addresses |  
| `All_combined_part3.md` lines 210323-210333 | Manual NAT and Unified NAT table | Manual NAT supports source/destination matching and is processed in Section 1 before Auto NAT unless `after-auto` is used |  
| `All_combined_part3.md` lines 210405-210407 | Advanced NAT options | Supports `no-proxy-arp` and route lookup behavior |  
| `All_combined_part3.md` lines 210526-210563 | Manual NAT identity NAT example | Supports `nat (inside,outside) source static LOCAL LOCAL destination static REMOTE REMOTE no-proxy-arp` |  
| `All_combined_part3.md` lines 225400-225458 | Site-to-site VPN NAT exemption | Confirms VPN traffic usually should bypass NAT, and crypto ACLs must match post-NAT addresses if NAT exemption is missing |  
| `All_combined_part3.md` lines 226018-226021 | VPN NAT exemption CLI | Supports `nat (inside,outside) ... source static Local-Net Local-Net destination static Remote-Net Remote-Net no-proxy-arp route-lookup` |  
| `All_combined_part3.md` lines 211214-211287 | NAT monitoring | Supports `show xlate`, `show nat`, and `show nat detail` for translation verification |  
...
Blunt rule: NAT exemption is not “no NAT rule.” It is an explicit identity NAT rule that must match before broad PAT. For VPN labs, build it before chasing IKE/IPsec ghosts.
$$